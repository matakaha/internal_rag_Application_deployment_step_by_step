...
このガイドは4つのステップで構成されています:

| Step | 内容 | 主な作業 | 所要時間 |
|------|------|---------|----------|
| **Step 01** | ACR構築 | Container Registry作成、NAT Gateway設定、Runnerイメージビルド | 15-20分 |
| **Step 02** | Subnet追加 | Container Instance用サブネット作成、NSG設定 | 5-7分 |
| **Step 03** | Key Vault構築 | Key Vault作成、シークレット格納、VPN DNS設定確認 | 10-15分 |
| **Step 04** | GitHub Actions設定 | GitHub Secrets設定、サンプルアプリデプロイ | 15-20分 |

**合計所要時間**: 約45-60分

> **💡 学習のコツ**: 
> - 各ステップは独立しているため、1日で完了させる必要はありません
> - Step 01-03は基盤構築、Step 04で実際のCI/CDを体験できます
> - 問題が発生した場合は各Stepのトラブルシューティングセクションを参照してください
...
**学習内容**: GitHub Actions、Self-hosted Runner動的作成、CI/CDパイプライン、OIDC認証
...
> **🚀 動的ACI作成**: GitHub ActionsワークフローでAzure Container Instanceを動的に作成・削除します。これによりコスト最適化と並列実行が可能になります。

#### 4-1. サンプルリポジトリを使用する場合（推奨）
...
# Container Instance一覧の確認(動的作成のため実行時のみ存在)
az container list \
  --resource-group $RESOURCE_GROUP \
